...
Feature: get and set the last login date
  ...
  Scenario Outline: Get the last login date of a user
    ...
    When the administrator gets the last login date for user "user1" using the testing API
  ...
  Scenario Outline: Set the last login date of a user
    Given using OCS API version "<ocs-api-version>"
    When the administrator sets the last login date for user "user1" to "7" days ago using the testing API
    And the administrator gets the last login date for user "user1" using the testing API
  ...
  Scenario Outline: Try to get the last login for a non-existent user
    Given using OCS API version "<ocs-api-version>"
    When the administrator gets the last login date for user "user2" using the testing API
    Then the OCS status message should be "user user2 not found"
  ...
  Scenario Outline: Try to set invalid last login date
    Given using OCS API version "<ocs-api-version>"
    When the administrator sets the last login date for user "user1" to "<number_of_days>" days ago using the testing API
